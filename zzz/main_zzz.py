...
bfplugin = zzz_ml+"/ZenlessZoneZero Game/ZenlessZoneZero_Data/Plugins/PCGameSDK.dll"
gfplugin= zzz_ml+"/ZenlessZoneZero Game/ZenlessZoneZero_Data/Plugins/PCGameSDK.dllx"
...
class zzz(QWidget):
    ...
    #     self.image_label.setGeometry(30, 300, 505, 285)

    def on_button_clicked_guanfu(self):
        if not is_admin():
            QMessageBox.critical(None, "错误", "请以管理员权限运行程序！")
            return
        with open(zzz_config, 'w') as file:
            file.write(G)
            if os.path.exists(bfplugin):
                os.remove(bfplugin)

        exe = zzz_ml + '/' + "ZenlessZoneZero Game/ZenlessZoneZero.exe"
        exe = '"' + exe + '"'
