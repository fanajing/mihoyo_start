...
bfplugin = xqtd_ml+"/Game/StarRail_Data/Plugins/PCGameSDK.dll"
gfplugin= xqtd_ml+"/Game/StarRail_Data/Plugins/PCGameSDK.dllx"
...
class xqtd(QWidget):
    ...
    def on_button_clicked_guanfu(self):
        if not is_admin():
            ...
            return
        with open(xqtd_config, 'w') as file:
            file.write(G)
            if os.path.exists(bfplugin):
                os.remove(bfplugin)

        exe = xqtd_ml + '/' + "Game/StarRail.exe"
        exe = '"' + exe + '"'
